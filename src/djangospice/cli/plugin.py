...
from .plugins import plugin_installer
from .plugins.generator import PluginGenerator, PluginConfig
...
def createplugin(name: str, path: str, official: bool, editable: bool):
    while True:  # loop for restart
        try:
            click.secho("djangospice Plugin Generator", fg="cyan", bold=True)
            ...
            author = ask("4 Enter author name", default="djangospice Team")
            author_email = ask("5 Enter author email", default="dev@djangospice.com", validator=validate_email)
...
def check():
    ...
    if not errors and not warnings:
        click.echo("djangospice Doctor: environment matches plugins.lock")
    else:
        if errors:
            click.echo("Errors:")
            for e in errors:
        ...
        if warnings:
            ...
            for w in warnings:
                click.echo("  " + w)
        raise click.ClickException("djangospice Doctor found issues")
