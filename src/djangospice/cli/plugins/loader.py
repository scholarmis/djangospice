...
from djangospice.container.exceptions import ServiceAlreadyRegisteredError
from djangospice.container import ServiceContainer
...
from .extensions import PinExtension, ValidationExtension
...
class PluginLoader:
    ...
    def __init__(self, service_registry: ServiceContainer):
    ...
    def discover_plugin(self, identifier: str) -> Optional[PluginMetadata]:
        ...
        for plugin in discovered:
            if identifier.lower() in (plugin.name.lower(), plugin.app.lower(), plugin.source.lower()):
                return plugin
    ...
    def load_plugin(self, plugin: PluginMetadata):
        ...
        try:
            app = importlib.import_module(plugin.app)
            self._register_plugin(plugin, app)
    ...
    def _fallback_load(self, plugin: PluginMetadata):
        try:
            ...
            if fs_plugin:
                ...
                try:
                    app = importlib.import_module(fs_plugin.app) 
                    self._register_plugin(fs_plugin, app)
                finally:
    ...
    def _register_plugin(self, plugin: PluginMetadata, app: Any):
    ...
    def unload_plugin(self, plugin_name: str):
        ...
        if plugin.app in sys.modules:
            del sys.modules[plugin.app]

    def start_all_services(self):
        """Delegate lifecycle start to ServiceContainer"""
        self.service_registry.start_singletons()

    def stop_all_services(self):
        """Delegate lifecycle stop to ServiceContainer"""
        self.service_registry.start_singletons()

    def register_services(self, plugin_module: Any):
        """
        Scans a loaded plugin app for contracts and their implementations
        and registers them with the ServiceContainer.
        """
    ...
    def unregister_services(self, plugin: PluginMetadata):
        """
        Unregisters services provided by the plugin from the ServiceContainer.
