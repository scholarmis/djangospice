...
from djarf.htmx.actions import BulkAction, RowAction
from djarf.htmx.download import render_download_launcher
from djarf.htmx.tasks import export_table_resource, export_model_resource
...
class HTMXExportableTableView(ExportMixin, HTMXTableView):
    ...
    def export_table(self, export_format, **kwargs):
        """
        ...
        export_table_resource.delay(
            user_id, model_path, table_path, export_format, **kwargs
        )
    
    def export_resource(self, **kwargs):
        """
        ...
        export_model_resource.delay(
            user_id, resource_path, export_data=True, **kwargs
        )
        ...
        if self.request.method == "POST" and is_htmx(self.request):
            ...
            if export_format in self.export_formats:
                if self.resource_class:
                    self.export_resource(**kwargs)
                else:
                    self.export_table(export_format, **kwargs)
