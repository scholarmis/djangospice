...
class GenericObjectRelatedField(serializers.RelatedField):
    """
    A custom field to dynamically handle GenericForeignKeys.
    Extracts the ID, model type, and string representation of any related object.
    """
...
class BaseModelSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        # Handle flatten_fields → replace FK IDs with string representations
        if action == "list":
            flatten_fields = getattr(self.Meta, "flatten_fields", [])
            for field_name in flatten_fields:
        ...
        # Determine which fields to include per action
        fields_to_use = None
        if action == "list":
            # Use list_fields if defined, fallback to all fields
            fields_to_use = getattr(self.Meta, "list_fields", None) or getattr(self.Meta, "fields", [])
            # Remove hidden_fields only in list
            hidden = getattr(self.Meta, "hidden_fields", [])
            data = {k: v for k, v in data.items() if k not in hidden and k in fields_to_use}
        elif action == "retrieve":
            fields_to_use = getattr(self.Meta, "detail_fields", None) or getattr(self.Meta, "fields", [])
            data = {k: v for k, v in data.items() if k in fields_to_use}
        elif action in ["create", "update", "partial_update"]:
            fields_to_use = getattr(self.Meta, f"{action}_fields", None) or getattr(self.Meta, "fields", [])
            data = {k: v for k, v in data.items() if k in fields_to_use}
        else:
            # fallback for any other action
            fields_to_use = getattr(self.Meta, "fields", [])
            data = {k: v for k, v in data.items() if k in fields_to_use}
...
class OptionModelSerializer(BaseModelSerializer):
    class Meta(BaseModelSerializer.Meta):
        fields = ["id", "name", "value", "slug", "code", "description", "is_visible"]
        abstract = True
